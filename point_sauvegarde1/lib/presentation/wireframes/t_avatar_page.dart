...
class _TAvatarPageState extends ConsumerState<TAvatarPage> {
  ...
  void initState() {
    super.initState();
    // Charger l'avatar actuel depuis le profil.
    final profile = ref.read(profileProvider);
    final current = profile.avatarId;
    // Format stocke : "avatar_N" -> index = N - 1
    if (current.startsWith('avatar_')) {
      final n = int.tryParse(current.substring(7));
      if (n != null && n >= 1 && n <= 12) {
        _selectedIndex = n - 1;
      }
    }
                  ...
                  child: DecoratedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: _selectedIndex != null
                          ? () async {
                              // Sauvegarder l'avatar en BDD + cache.
                              // Format : "avatar_N" ou N est 1-based.
                              final avatarId = 'avatar_${_selectedIndex! + 1}';
                              await ref
