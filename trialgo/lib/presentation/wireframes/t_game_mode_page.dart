...
// pour corriger l'invisibilite des textes en mode clair (issue connue).
...
class _TGameModePageState extends State<TGameModePage>
  ...
  Widget build(BuildContext context) {
    // Gradient de fond theme-aware : utilise TBrand.bgDark en mode
    // sombre, TBrand.bgLight en mode clair. Le reste du design
    // (particules, vignette, cartes, boutons) reste inchange.
    ...
    return Scaffold(
      ...
      body: Stack(
        ...
        children: [
          ...
          // =============================================================
          // Le fond utilise le degrade theme-aware choisi ci-dessus.
          ...
          // =============================================================
          // COUCHE 2 : EFFET VIGNETTE (bords plus contrastes)
          // =============================================================
          // Un degrade radial transparent au centre, assombri/eclaircit
          // sur les bords selon le theme. En dark : bords noirs (cine).
          // En light : bords tres legerement sombres pour le contraste
          // mais sans effet "ecran eteint" qui serait desagreable.
          Positioned.fill(
            child: IgnorePointer(
              child: DecoratedBox(
  ...
  Widget _buildHeader() {
    ...
    // c'est sombre. Toutes les opacites sont preservees pour garder
    // le look "glass" / "ghost" sans casser la lisibilite.
  ...
  Widget _buildDecorativeLine() {
    // On passe la couleur au Painter pour qu'elle s'adapte au theme :
    // sinon la ligne blanche @ 20% alpha disparait sur fond clair.
    final stroke = TColors.of(context).textPrimary.withValues(alpha: 0.2);
    return SizedBox(
      width: 200,
      height: 20,
      child: CustomPaint(
        painter: _DiamondLinePainter(color: stroke),
      ),
    );
  }
  ...
  Widget _buildModeCard({
    ...
    // Couleurs theme-aware : la carte doit etre lisible dans les 2
    // themes. En dark la base est bleu-violet profond (aspect "table
    // de jeu"). En light, c'est blanc casse avec des ombres legeres
    // (aspect "carte posee sur table claire").
    final colors = TColors.of(context);
    final isDarkTheme = Theme.of(context).brightness == Brightness.dark;
    final cardBgColor =
        isDarkTheme ? const Color(0xFF1E1E3A) : Colors.white;
    final cardBorderColor = isDarkTheme
        ? Colors.white.withValues(alpha: 0.08)
        : Colors.black.withValues(alpha: 0.08);

    // Foreground theme-aware : textPrimary est blanc en dark et sombre
    // en light. Les opacites conservent la hierarchie visuelle sans
    // rendre les textes invisibles sur fond blanc (bug d'origine en
    // mode clair : tout etait Colors.white -> blanc sur blanc).
    final fg = colors.textPrimary;
    final fgSecondary = colors.textSecondary;
    final fgTertiary = colors.textTertiary;
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      style: GoogleFonts.rajdhani(
                        ...
                        fontWeight: FontWeight.w900,
                        letterSpacing: 4,
                        // Quand selectionnee : titre pleinement visible (fg).
                        // Sinon : attenue mais lisible (fgSecondary).
                    ...
                    Text(
                      ...
                      style: GoogleFonts.exo2(
                        ...
                        color: isSelected ? accentColor : fgTertiary,
                      ),
                    ),
                    ...
                    Text(
                      ...
                      style: GoogleFonts.exo2(
                        fontSize: 10,
                        color: isSelected ? fgSecondary : fgTertiary,
                        height: 1.3,
                      ),
                    ),
  ...
  Widget _buildShimmerButton() {
    ...
    // Cas "aucun mode" : on retombe sur un gris neutre mais visible
    // dans les 2 themes (sombre en dark, clair en light) via TColors.
    final colors = TColors.of(context);
    final neutralDark = colors.borderStrong;
    final neutralDarker = colors.borderDefault;
    final gradient = _selectedMode == 0
        ...
        : _selectedMode == 1
            ...
            : LinearGradient(colors: [neutralDark, neutralDarker]);
...
class _DiamondLinePainter extends CustomPainter {
  /// Couleur de la ligne + du losange. Passee theme-aware par l'appelant
  /// (sinon la ligne blanche disparait sur fond clair).
  final Color color;

  _DiamondLinePainter({required this.color});
