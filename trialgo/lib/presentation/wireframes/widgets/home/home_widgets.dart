...
class HomeQuickStats extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // --- Derniere session jouee (source de verite = Supabase) ---
    //
    // profile.lastSession lit la premiere entree de recentSessions,
    // qui est rechargee dans reload() et mise a jour a chaque
    // recordGameSession(). Plus aucun MockData ici.
    //
    // Cas sans historique (joueur tout juste inscrit) : on affiche
    // "-/-" pour indiquer qu'il n'a pas encore joue de partie.
    final last = profile.lastSession;
    final String lastSessionLabel;
    if (last == null) {
      lastSessionLabel = '-/-';
    } else {
      // Format "6/8" = bonnes reponses / total questions.
      lastSessionLabel = '${last.correctAnswers}/${last.questionsTotal}';
    }
